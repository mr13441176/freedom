...
class AXI4AsMemIODDR(depth : BigInt) extends GenericParameterizedBundle(depth) {
...
trait AXI4AsMemIOClocksReset extends Bundle {
  //inputs
...
class fuckingvc707mig(depth : BigInt)(implicit val p:Parameters) extends BlackBox
{
  ...
  val io = new AXI4AsMemIODDR(depth) with AXI4AsMemIOClocksReset {
    // User interface signals
