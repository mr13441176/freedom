...
package uec.keystoneAcc.fpgashells.shell.xilinx
...
import uec.keystoneAcc.freedom.unleashed.KeyIsPCIe
class VC707Shell()(implicit p: Parameters) extends Series7Shell
{
  ...
  val pcie      = if(p(KeyIsPCIe))
                  Overlay(PCIeOverlayKey)      (new PCIeVC707Overlay    (_, _, _))
                  else None
